...
1. 下载 `fnytdlp.fpk`（约 76.6 MB，v0.7.1）
2. 在飞牛NAS应用中心 → 手动安装
...
**质量预设/容器格式点击修复 + 单链接质量下拉框 + FFmpeg 路径设置**

- **🐛 修复质量预设/容器格式按钮点击无反应** — 补齐 `.quality-btn`/`.container-btn` 缺失的 CSS（基础/hover/选中态），并加 DOM 0 级直接绑定兜底（fnOS WebView inline onclick 失效）
- **⬇️ 单链接下载质量下拉框** — 粘单个链接解析后，根据解析数据自动生成下拉框（实际分辨率/音频档位），用户点选即下载对应质量；video-only 自动拼最佳音频、combined 用自身格式
- **🎞️ FFmpeg 路径设置 + 自动检测** — 设置→高级新增 FFmpeg 路径输入框与 🔍 自动检测按钮，检测到即自动保存；转码/合并时 yt-dlp 收到 `--ffmpeg-location` 调用指定路径
- **🗑 移除首页视频/音频分类 tab** — 按扩展名筛选对下载中的任务（ext 为空）无效，两个分类形同虚设，已删除（状态分类全部/下载中/已完成/出错/暂停/取消 保留）

### v0.7.1 (2026-09-19)

**6 项对标优化 + 联系作者**

- **🏷️ 播放列表"只选未下载项"** — `/api/playlist` 读取下载档案，已下载项灰显 + ✓ 徽标 + 默认不勾选，新增"只选未下"快捷按钮
- **🏷️ 已下载质量徽标** — 格式选择时扫描下载目录匹配视频 ID，已存在的格式打 ✓ 徽标并灰显
- **⚠️ 歧义 URL 显式处理** — 粘贴 `watch?v=X&list=Y` 弹「下载单视频 / 下载整个列表」双按钮
- **🗂 下载文件浏览器** — 顶部新增 🗂 按钮，独立弹窗列出下载目录文件，支持搜索 / 播放（流式）/ 删除
- **🚦 下载队列并行度** — 并发下载达到上限时任务自动排队，spawn 失败也会释放并发槽调度下一个
- **🎬 播放列表编码预设** — VP9+Opus / AV1+Opus / H.264+AAC 三档偏好，替代单一"最高质量"
- **✉️ 联系作者** — 顶部新增联系作者按钮，弹窗显示 QQ 群号 745565752 支持一键复制
